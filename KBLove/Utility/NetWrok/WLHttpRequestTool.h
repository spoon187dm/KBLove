 ...
 *  网络请求类型(GET,POST)
 */
typedef NS_ENUM(NSInteger, WLHttpRequestType) {
    /**
     *  GET请求
     */
    WLHttpRequestTypeGet,
    /**
     *  POST请求
     */
...
typedef void (^WLHttpstatueBlock)(BOOL isconnect, WLHttpConnectStatue httpState);
typedef void (^WLHttpRequestFiniedBlock)(BOOL IsSuccess, id result);
typedef WLCacheStrategy* (^WLHttpCacheBlock)(BOOL isStrategyLegol);

@interface WLHttpRequestTool : NSObject
