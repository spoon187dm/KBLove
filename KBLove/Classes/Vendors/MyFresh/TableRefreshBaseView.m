#import "TableRefreshBaseView.h"
#import "TableViewFreshConst.h"
#import "UIView+Extension.h"
#import "UIScrollView+Extension.h"
#import <objc/message.h>

@interface  TableRefreshBaseView()
{
...
@implementation TableRefreshBaseView
#pragma mark - 控件初始化
/**
 *  状态标签
 */
- (UILabel *)statusLabel
{
    if (!_statusLabel) {
        ...
        statusLabel.textColor = TableRefreshLabelTextColor;
...
- (instancetype)initWithFrame:(CGRect)frame {
    frame.size.height = TableRefreshViewHeight;
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if (self.superview) { // 旧的父控件
        [self.superview removeObserver:self forKeyPath:TableRefreshContentOffset context:nil];
    }
    
    if (newSuperview) { // 新的父控件
        [newSuperview addObserver:self forKeyPath:TableRefreshContentOffset options:NSKeyValueObservingOptionNew context:nil];
...
{
    if (self.state == TableRefreshStateWillRefreshing) {
...
- (BOOL)isRefreshing
{
    return TableRefreshStateRefreshing == self.state;
}
