...
//  TableRefresh
//
//  Created by Table on 13-2-26.
//  Copyright (c) 2013年 itcast. All rights reserved.
//  上拉加载更多

#import "TableRefreshFooterView.h"
#import "TableViewFreshConst.h"
...
+ (instancetype)footer
{
    return [[TableRefreshFooterView alloc] init];
}
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if (self.superview) { // 旧的父控件
        [self.superview removeObserver:self forKeyPath:TableRefreshContentSize context:nil];
    }
    
    if (newSuperview) { // 新的父控件
        // 监听
        [newSuperview addObserver:self forKeyPath:TableRefreshContentSize options:NSKeyValueObservingOptionNew context:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([TableRefreshContentSize isEqualToString:keyPath]) {
        // 调整frame
        [self adjustFrameWithContentSize];
...
- (void)adjustStateWithContentOffset
{
    ...
    } else if (self.state == TableRefreshStatePulling) {// 即将刷新 && 手松开
        // 开始刷新
...
- (void)setState:(TableRefreshState)state
{
    ...
    // 2.保存旧状态
    TableRefreshState oldState = self.state;
	...
	switch (state)
    {
        ...
        {
            // 设置文字
            self.statusLabel.text = TableRefreshFooterPullToRefresh;
            
            // 刷新完毕
            if (TableRefreshStateRefreshing == oldState) {
                self.arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
                [UIView animateWithDuration:TableRefreshSlowAnimationDuration animations:^{
            ...
            // 刚刷新完毕
            if (TableRefreshStateRefreshing == oldState && deltaH > 0 && currentCount != self.lastRefreshCount) {
        ...
        {
            // 设置文字
            self.statusLabel.text = TableRefreshFooterReleaseToRefresh;
        ...
        {
            // 设置文字
            self.statusLabel.text = TableRefreshFooterRefreshing;
