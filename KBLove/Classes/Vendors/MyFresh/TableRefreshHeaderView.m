...
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        lastUpdateTimeLabel.textColor = TableRefreshLabelTextColor;
        ...
        // 2.加载时间
        self.lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:TableRefreshHeaderTimeKey];
    }
...
+ (instancetype)header
{
    return [[TableRefreshHeaderView alloc] init];
}
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:TableRefreshHeaderTimeKey];
...
- (void)setState:(TableRefreshState)state
{
    ...
    // 4.根据状态执行不同的操作
	switch (state) {
		case TableRefreshStateNormal: // 下拉可以刷新
        {
            // 设置文字
		...
		case TableRefreshStatePulling: // 松开可立即刷新
        {
            // 设置文字
		...
		case TableRefreshStateRefreshing: // 正在刷新中
        {
            // 设置文字
