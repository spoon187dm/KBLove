...
- (UIView *)overLayView:(OverLayView *)view didHitPoint:(CGPoint)didHitPoint withEvent:(UIEvent *)withEvent{
    ...
    //用于防止手势不响应
    CGPoint point = [self.view convertPoint:location fromView:self.activeCell];
    point.y = 40;
    NSLog(@"%lf,%lf",point.x,point.y);
    return (shoudReceivePointTouch) ? [self.activeCell hitTest:point withEvent:withEvent] : view;
}

- (void)hideMenuActive:(BOOL)aninated{
    __block TableMenuViewController *tableViewMenu = self;
