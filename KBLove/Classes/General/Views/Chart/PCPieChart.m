...
//        绘制饼状图
		...
		for (int i=0; i<[tmpComponents count]; i++)
		{
			...
			if (nextStartDeg > 180 ||  (nextStartDeg < 180 && endDeg> 270) )
			{
				...
				CGSize optimumSize = [percentageText boundingRectWithSize:CGSizeMake(max_text_width, 100) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:self.percentageFont} context:nil].size;
...
//								CGContextFillPath(ctx);
                ...
                optimumSize = [percentageText boundingRectWithSize:CGSizeMake(max_text_width, 100) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:self.titleFont} context:nil].size;
...
//				[component.title drawInRect:titleFrame withFont:self.titleFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
                ...
                [percentageText drawInRect:titleFrame withAttributes:@{NSFontAttributeName:self.percentageFont,
                                                                      NSParagraphStyleAttributeName:style,
                                                                      NSForegroundColorAttributeName:component.colour}];
...
//                设置字体属性
				...
				{
					// draw line to point to chart
//                    [UIColor colorWithRed:0.905 green:0.000 blue:0.000 alpha:1.000];
					CGContextSetRGBStrokeColor(ctx, 1.0f, 1.0f, 1.0f, 1.0f);
                    CGContextSetRGBFillColor(ctx, 1.0f, 1.0f, 1.0f, 1.0f);
					//CGContextSetRGBStrokeColor(ctx, 1.0f, 1.0f, 1.0f, 1.0f);
					//CGContextSetRGBFillColor(ctx, 1.0f, 1.0f, 1.0f, 1.0f);
