...
{
    CLLocationCoordinate2D coor = [MapUtils geoPoint2Coordinate2D:p];
...
+(BMKGeoPoint)wgs84ToBaiduPoint:(BMKGeoPoint)p
{
    CLLocationCoordinate2D coor = [MapUtils geoPoint2Coordinate2D:p];
    NSDictionary *Dic_fixlocation = BMKConvertBaiduCoorFrom(coor,BMK_COORDTYPE_GPS);
    return [MapUtils coordinate2D2geoPoint:BMKCoorDictionaryDecode(Dic_fixlocation)];
}
...
{
    BMKGeoPoint baiduPoint = [MapUtils wgs84ToBaiduPoint:point];
...
{
    CLLocationCoordinate2D startCoord = [MapUtils geoPoint2Coordinate2D:p1];
    BMKMapPoint point1 = BMKMapPointForCoordinate(startCoord);
    
    CLLocationCoordinate2D endCoord = [MapUtils geoPoint2Coordinate2D:p2];
...
{
    CLLocationCoordinate2D startCoord = [MapUtils geoPoint2Coordinate2D:start];
    BMKMapPoint point1 = BMKMapPointForCoordinate(startCoord);
    
    CLLocationCoordinate2D endCoord = [MapUtils geoPoint2Coordinate2D:end];
    BMKMapPoint point2 = BMKMapPointForCoordinate(endCoord);
    ...
    return [MapUtils coordinate2D2geoPoint:targetCoord];
}

+(BMKGeoPoint) getCenterPoint:(BMKGeoPoint)start end:(BMKGeoPoint)end
{
    return [MapUtils getPositionByRatio:start end:end ratio:0.5];
}
...
+(void) adjustMapCenterAndSpan:(BMKMapView*)mapView statusInfo:(NSArray*) statusInfo // CCDeviceStatus
{
    ...
    [MapUtils adjustMapCenterAndSpan:mapView start:[MapUtils geoPoint2Coordinate2D:topLeft] end:[MapUtils geoPoint2Coordinate2D:bottomRight]];
}
...
+(void) adjustMapCenterAndSpan:(BMKMapView*)mapView start:(CLLocationCoordinate2D) start end:(CLLocationCoordinate2D)end
{
    ...
    CLLocationCoordinate2D center = [MapUtils findCenterPoint:start _loc2:end];
                             ...
                             atBearingDegrees:(double)bearingDegrees
{
    ...
    double bearingRadians = [MapUtils radiansFromDegrees:bearingDegrees];
    double fromLatRadians = [MapUtils radiansFromDegrees:fromCoord.latitude];
    double fromLonRadians = [MapUtils radiansFromDegrees:fromCoord.longitude];
...
+ (BMKGeoPoint)geoPointFromPoint:(BMKGeoPoint)fromPoint
                             ...
                             atBearingDegrees:(double)bearingDegrees
{
    CLLocationCoordinate2D fromCoord = [MapUtils geoPoint2Coordinate2D:fromPoint];
    CLLocationCoordinate2D toCoord = [MapUtils coordinateFromCoord:fromCoord atDistanceKm:distanceKm atBearingDegrees:bearingDegrees];
    return [MapUtils coordinate2D2geoPoint:toCoord];
}
