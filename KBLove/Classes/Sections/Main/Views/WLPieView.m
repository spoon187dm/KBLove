...
-(id)initWithFrame:(CGRect)frame andTotal:(NSNumber *)total andCurrent:(NSNumber *)current andClockwise:(BOOL)clockwise andShadow:(BOOL)hasBackgroundShadow andBgColor:(UIColor *)color andStart:(CGFloat)start andEnd:(CGFloat)end andredius:(CGFloat)radius andWidth:(CGFloat)width andLabel:(BOOL)isLabel
{
    ...
    if (self) {
        ...
        _current = current;
        _strokeColor = color;
        _strokeColorGradientStart = color;
        _duration = 1.0;
        ...
        UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.center.x, self.center.y) radius:radius startAngle:DEGREES_TO_RADIANS(startAngle) endAngle:DEGREES_TO_RADIANS(endAngle) clockwise:clockwise];
...
- (void)strokeChart
{
    ...
    if (_strokeColorGradientStart) {
        ...
        gradientMask.lineCap = kCALineCapSquare;
